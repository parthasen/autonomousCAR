...
class Vehicle(object):
  ...
  def __init__(self, lane, s, v, a):
    self.lane = lane
    self.s = s        # s: distance along the road
    self.v = v        # velocity
  ...
  def _trajectory_for_state(self,state,predictions, horizon=5):
    '''  Private method: calculate Trajectory for State, given Predictions
    '''
  ...
  def snapshot(self):
    '''  Return current Snapshot (lane, s, velocity, acceleration, state)
    '''
