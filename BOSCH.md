...
#### Sensor Fusion
Its important that the car doesn't crash into any of the other traffic on the road which is all going at different speeds around the speed limit and can change lanes.
...
The vx, vy values can be really useful for predicting where the cars will be in the future along the road. For instance if you were to assume that the tracked car keeps moving along the road then its future predicted Frenet s value will be its current s value plus its total velocity (m/s) multiplied by the time out in the future (s).

#### Changing lanes
The last consideration to make is how to create paths that can smoothly changes lanes. Anytime the car approaches a car in front of it that is moving slower than the speed limit, the car should consider if it should change lanes. The car should only change lanes if its safe, also if the lane change will help it move through the flow of traffic better. A lane change path should result in low acceleration, jerk and optimize its distance away from other traffic for maximum safety. The acceleration and jerk part can be solved from linear equations for s and d functions, one example of this is shown in python below. Using the provided Eigen-3.3 library for instance can help solve such linear equations and getXY can be used to transform (s,d) points to (x,y) points for the returned path. Review the Quintic Polynomial Solver and Polynomial Trajectory quizzes for reference.
